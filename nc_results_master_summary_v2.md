...
**注**: 装机容量基于实测屋顶面积 (`d2_3_roof_area_total_m2`)，经屋顶利用率 (0.60) 和组件功率密度 (0.15 kWp/m²) 转换。如改用经遮挡修正的 `d3_4_total_deployable_mw` 精度更高，数字可能略有变化。
...
### 屋顶面积数据（已确认）
使用 `d2_3_roof_area_total_m2`（样本区所有住宅建筑屋顶总面积，直接求和，范围 0.45–11.8 km²）。经遮挡修正的可部署面积 `d3_4_total_deployable_m2`（0.29–7.1 km²）和直接装机容量 `d3_4_total_deployable_mw`（58–1418 MW）也可用，精度更高。
...
- [x] ~~屋顶面积估算方法~~ → 使用真实求和字段 d2_3_roof_area_total_m2
- [x] ~~港台 PVGIS 数据源~~ → 全部 PVGIS-ERA5
- [x] ~~是否加澳门~~ → 不加（见第9节）
- [x] ~~港台政治表述~~ → 见第9节规范
- [ ] 装机容量是否改用 d3_4_total_deployable_mw（经遮挡修正，更准确）→ 需重跑 02d
- [ ] 情景分析是否需要对 41 城重跑（目前基于 39 城数据）
- [ ] 导师对港台表述的最终审核
